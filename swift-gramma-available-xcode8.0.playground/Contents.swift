...
/********/
var individualScore = [10,9,33,1,11]
var teamScore = 0
...
nameAndDay = greeet(name: "wangggang", day: "27", number: 12)
print(nameAndDay)
...
let statistics = calculate(scores: [1,5,17,6])
print(statistics)
print(statistics.1)
...
func averageSumOfSquare(a:Float,b:Float) -> Float{
    return (square(a: a) + square(a: b))/2
}

func averageSumOfCube(a:Float,b:Float) -> Float
{
    return (cube(a: a)+cube(a: b))/2
}

averageSumOfCube(a: 2, b: 3)
averageSumOfCube(a: 1, b: 2)

/***函数作为参数**/
func averageSum(a:Float,b:Float,f:(Float -> Float)) -> Float{
    return(f(a) + f(b))/2.0
}
averageSum(a: 2.0, b: 3.0, f: square)
averageSum(a: 3.0, b: 3.0, f: cube)

let  averageNewSum = averageSum(a: 2, b: 3, f: {(x:Float)-> Float in return x*x})
print(averageNewSum)
...
hasAnyMathes(list: numbers, condition: lessThanTen)
...
print(unsafeAddress(of: arrayValue))
print(unsafeAddress(of: arrayCopyValue)
)
...
swapTwoInts(a: &someInt, &someIntTwo)
print(someIntTwo)
...
swapTwoValues(a: &oneGenericValue, &twoGenericValue)
swapTwoValues(a: &someInt, &someIntTwo)
...
stackOfStrings.push(item: "thres")
